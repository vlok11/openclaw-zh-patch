...
class TurnContext {
    ...
    async sendActivity(activityOrText, speak, inputHint) {
        ...
        if (typeof activityOrText === 'string') {
            activity = { type: agents_activity_1.ActivityTypes.Message, text: activityOrText, inputHint: inputHint || agents_activity_1.InputHints.AcceptingInput };
            if (speak) {
    ...
    async sendActivities(activities) {
        ...
        const output = activities.map((activity) => {
            const newActivity = agents_activity_1.Activity.fromObject(activity);
            ...
            if (result.type === agents_activity_1.ActivityTypes.InvokeResponse) {
                this.turnState.set(activityHandler_1.INVOKE_RESPONSE_KEY, result);
            }
