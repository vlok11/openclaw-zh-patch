...
import { A as escapeRegExp, m as getActivePluginRegistry, q as isPlainObject } from "./subsystem-BXiyX1bx.js";
import { Sn as DEFAULT_PROVIDER, u as resolveConfiguredModelRef, xn as DEFAULT_MODEL } from "./model-selection-Dof4G7X1.js";
import { n as listChannelDocks } from "./dock-kBiZJ0I4.js";
...
function buildChatCommands() {
	const commands = [
		...
		defineChatCommand({
			key: "session",
			nativeName: "session",
			description: "管理会话级设置（例如 /session ttl）。",
			textAlias: "/session",
			category: "session",
			args: [{
				name: "action",
		...
		defineChatCommand({
			key: "focus",
			nativeName: "focus",
			description: "将此 Discord 线程（或新线程）绑定到会话目标。",
			textAlias: "/focus",
			category: "management",
			args: [{
				name: "target",
