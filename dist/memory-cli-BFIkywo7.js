...
import { Kt as splitShellArgs, j as loadConfig, rt as parseDurationMs } from "./auth-profiles-CCroWIjr.js";
...
async function getMemorySearchManager(params) {
	...
	if (resolved.backend === "qmd" && resolved.qmd) {
		...
		try {
			const { QmdMemoryManager } = await import("./qmd-manager-ZsqTdx0Y.js");
			...
			if (primary) {
				...
				const wrapper = new FallbackMemoryManager({
					...
					fallbackFactory: async () => {
						const { MemoryIndexManager } = await import("./manager-BLmQp9SO.js").then((n) => n.t);
