...
function isPathInside$2(root, target) { return true; 
...
async function assertSchtasksAvailable() {
	...
	throw new Error(`schtasks unavailable: ${detail || "unknown error"}. On Windows, run in PowerShell/CMD or skip daemon install.`.trim());
}
async function installScheduledTask({ env, stdout, programArguments, workingDirectory, environment, description }) {
	...
	if (create.code !== 0) {
		...
		const hint = /access is denied/i.test(detail) ? " 请以管理员身份运行 PowerShell，或跳过守护进程安装重新运行。" : "";
...
function renderSystemdUnavailableHints(options = {}) {
	...
	return ["systemd 用户服务不可用；请安装/启用 systemd 或在你的进程管理器下运行网关。", `如果你在容器中，请在前台运行网关而不是使用 \`${formatCliCommand("openclaw gateway")}\`。`];
}
