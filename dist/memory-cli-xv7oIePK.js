...
import { Bt as loadConfig, fn as parseDurationMs, tr as splitShellArgs } from "./model-selection-BBF__Y3p.js";
...
async function getMemorySearchManager(params) {
	...
	if (resolved.backend === "qmd" && resolved.qmd) {
		...
		try {
			const { QmdMemoryManager } = await import("./qmd-manager-DVgCW1KI.js");
			...
			if (primary) {
				...
				const wrapper = new FallbackMemoryManager({
					...
					fallbackFactory: async () => {
						const { MemoryIndexManager } = await import("./manager-CFRkrYbL.js").then((n) => n.t);
