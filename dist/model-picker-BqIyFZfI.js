...
import { N as resolveAgentModelPrimaryValue } from "./agent-scope-B8pKj4S2.js";
import { Dr as listProfilesForProvider, Nr as ensureAuthProfileStore, X as getCustomProviderApiKey, c as modelKey, et as resolveEnvApiKey, jr as upsertAuthProfileWithLock, m as resolveConfiguredModelRef, ni as DEFAULT_MODEL, r as buildModelAliasIndex, ri as DEFAULT_PROVIDER, t as buildAllowedModelSet, u as normalizeProviderId } from "./model-selection-BBF__Y3p.js";
import { n as loadModelCatalog } from "./model-catalog-CdZtthLU.js";
import { i as formatTokenK } from "./shared-DhQjD8uP.js";
...
function addModelSelectOption(params) {
	...
	if (params.entry.reasoning) hints.push("推理");
	const aliases = params.aliasIndex.byKey.get(key);
	if (aliases?.length) hints.push(`alias: ${aliases.join(", ")}`);
	if (!params.hasAuth(params.entry.provider)) hints.push("未认证");
	params.options.push({
		value: key,
		label: key,
...
async function promptModelAllowlist(params) {
	...
	if (catalog.length === 0 && allowedKeys.length === 0) {
		const raw = await params.prompter.text({
			message: params.message ?? "允许的模型（逗号分隔 提供商/模型名；留空保持当前）",
