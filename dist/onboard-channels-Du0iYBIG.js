...
import { dt as normalizeDiscordSlug } from "./send-DvyC7dym.js";
import { Fn as resolveSlackUserAllowlist, In as resolveDiscordUserAllowlist, T as resolveDiscordChannelAllowlist, _ as loadOpenClawPlugins, g as createPluginLoaderLogger, y as resolveSlackChannelAllowlist } from "./subagent-registry-gheT8zqW.js";
import { a as listChannelPluginCatalogEntries, n as isChannelConfigured } from "./plugin-auto-enable-xva3bT1N.js";
import { s as normalizeIMessageHandle } from "./send-DgwnbcsQ.js";
import { t as formatDocsLink } from "./links-yQMT2QcX.js";
import { r as detectBinary } from "./onboard-helpers-9g7UgWKP.js";
...
import { a as extractArchive } from "./install-safe-path-D1LKWNB5.js";
import { n as recordPluginInstall, r as installPluginFromNpmSpec, t as buildNpmResolutionInstallFields } from "./installs-Cu3BCURZ.js";
...
async function promptInstallChoice(params) {
	...
	const options = [
		{
			value: "npm",
			label: `从 npm 下载 (${entry.install.npmSpec})`
		},
		...localOptions,
		{
			value: "skip",
...
async function setupChannels(cfg, runtime, prompter, options) {
	...
	const resolveDisabledHint = (channel) => {
		const plugin = getChannelPlugin(channel);
		if (!plugin) {
			if (next.plugins?.entries?.[channel]?.enabled === false) return "插件已禁用";
		...
		return enabled === false ? "已禁用" : void 0;
	};
	...
	const ensureBundledPluginEnabled = async (channel) => {
		...
		next = result.config;
		if (!result.enabled) {
			await prompter.note(`无法启用 ${channel}：${result.reason ?? "插件已禁用"}。`, "Channel setup");
			return false;
		}
		...
		if (!getChannelPlugin(channel)) {
			if (getChannelOnboardingAdapter(channel)) {
				await prompter.note(`${channel} 插件不可用（继续初始化向导）。如果设置完成后渠道仍无法工作，请运行 \`${formatCliCommand("openclaw plugins list")}\` and \`${formatCliCommand("openclaw plugins enable " + channel)}\`, 然后重启网关。`, "Channel setup");
