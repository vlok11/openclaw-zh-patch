import { Ct as resolveSecretRefString, et as resolveEnvApiKey, kt as encodeJsonPointerToken, on as isValidFileSecretRefId, p as resolveAllowlistModelKey, ri as DEFAULT_PROVIDER, sn as resolveDefaultSecretProviderAlias } from "./model-selection-BBF__Y3p.js";
...
async function applyDefaultModelChoice(params) {
	if (params.setDefaultModel) {
		...
		if (params.noteDefault) await params.prompter.note(`默认模型已设置为 ${params.noteDefault}`, "模型已配置");
		return { config: next };
	}
...
async function resolveApiKeyRefForOnboarding(params) {
	...
	while (true) {
		...
		if (source === "env") {
			...
			await params.prompter.note(`Validated environment variable ${envVar}. OpenClaw will store a reference, not the key value.`, "引用已验证");
			return {
				ref,
				resolvedValue
			};
		...
		if (externalProviders.length === 0) {
			await params.prompter.note("尚未配置文件/exec 密钥提供商。请在 secrets.providers 下添加，或选择环境变量。", "未配置提供商");
			continue;
		}
		...
		if (!providerEntry || providerEntry.source !== "file" && providerEntry.source !== "exec") {
			await params.prompter.note(`Provider "${selectedProvider}" is not a file/exec provider.`, "无效的提供商");
			continue;
		}
		...
		try {
			...
			await params.prompter.note(`Validated ${providerEntry.source} reference ${selectedProvider}:${id}. OpenClaw will store a reference, not the key value.`, "引用已验证");
			return {
				ref,
				resolvedValue
			};
...
function createAuthChoiceAgentModelNoter(params) {
	return async (model) => {
		...
		await params.prompter.note(`默认模型已设置为 ${model}，用于智能体 "${params.agentId}"。`, "模型已配置");
	};
}
...
async function resolveSecretInputModeForEnvSelection(params) {
	...
	return await params.prompter.select({
		message: "你想如何提供此 API 密钥？",
		initialValue: "plaintext",
		options: [{
			value: "plaintext",
			label: "直接粘贴 API 密钥",
			hint: "将密钥直接存储在 OpenClaw 配置中"
		}, {
			value: "ref",
			label: "使用密钥引用",
			hint: "存储对环境变量或已配置的外部密钥提供商的引用"
		}]
	}) === "ref" ? "ref" : "plaintext";
}
