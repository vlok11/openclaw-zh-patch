...
function gatewayInstallErrorHint(platform = process.platform) {
	return platform === "win32" ? "提示：请以管理员身份运行 PowerShell（开始 → 输入 PowerShell → 右键 → 以管理员身份运行），或跳过服务安装。" : `Tip: rerun \`${formatCliCommand("openclaw gateway install")}\` after fixing the error.`;
}
