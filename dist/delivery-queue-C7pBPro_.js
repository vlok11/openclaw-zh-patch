...
const MAX_RETRIES = 8;
/** Exponential Backoff delays in milliseconds with slight aggressive jitter index. */
const BACKOFF_MS = [
	2e3,   // 2s
	5e3,   // 5s
	15e3,  // 15s
...
/** Compute the backoff delay in ms for a given retry count with anti-thundering heap jitter. */
function computeBackoffMs(retryCount) {
	if (retryCount <= 0) return 0;
	const baseDelay = BACKOFF_MS[Math.min(retryCount - 1, BACKOFF_MS.length - 1)] ?? BACKOFF_MS.at(-1) ?? 0;
