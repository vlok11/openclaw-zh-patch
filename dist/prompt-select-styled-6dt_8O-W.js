...
import { $r as DEFAULT_MODEL, Fn as getModelRefStatus, Kn as resolveHooksGmailModel, L as writeConfigFile, N as readConfigFileSnapshot, Or as resolveApiKeyForProvider, Wn as resolveConfiguredModelRef, bn as ensureAuthProfileStore, ei as DEFAULT_PROVIDER, fn as repairOAuthProfileIdMismatch, l as resolveApiKeyForProfile, ni as CLAUDE_CLI_PROFILE_ID, ri as CODEX_CLI_PROFILE_ID, s as resolveProfileUnusableUntilForDisplay, wn as updateAuthProfileStoreWithLock } from "./auth-profiles-CCroWIjr.js";
import { t as formatCliCommand } from "./command-format-CDHXQh1_.js";
import { a as resolveAgentDir, d as resolveDefaultAgentId, g as DEFAULT_AGENTS_FILENAME, u as resolveAgentWorkspaceDir } from "./agent-scope-Cqn0zzij.js";
import { w as parseAgentSessionKey } from "./session-key-qmb11-mH.js";
import { t as resolveOpenClawPackageRoot } from "./openclaw-root-DlBiBbD9.js";
...
import { B as cleanStaleLockFiles, at as formatSessionArchiveTimestamp, c as loadSessionStore, ot as isPrimarySessionTranscriptFileName, tt as resolveMainSessionKey } from "./sessions-BI3YGZp9.js";
import { n as listChannelPlugins } from "./plugins-L6ij92Mg.js";
import { _ as loadOpenClawPlugins, en as DEFAULT_OAUTH_WARN_MS, nn as formatRemainingShort, tn as buildAuthHealthSummary, zn as resolveAgentSessionDirs } from "./subagent-registry-gheT8zqW.js";
...
import { n as callGateway, t as buildGatewayConnectionDetails } from "./call-CxwFCooR.js";
import { r as isLoopbackHost, u as resolveGatewayBindHost } from "./net-DBrsyv8q.js";
import { D as DEFAULT_SANDBOX_COMMON_IMAGE, E as DEFAULT_SANDBOX_BROWSER_IMAGE, O as DEFAULT_SANDBOX_IMAGE, p as resolveSandboxScope } from "./sandbox-DkcIu9cl.js";
...
import { i as resolveMemoryBackendConfig } from "./memory-cli-BFIkywo7.js";
...
import { l as resolveDmAllowState } from "./pi-tools.policy-Dp2cgsNC.js";
import { r as isWSLEnv, t as isWSL } from "./wsl-D9-kubbW.js";
import { h as randomToken, n as applyWizardMetadata, p as printWizardHeader, s as guardCancel } from "./onboard-helpers-9g7UgWKP.js";
...
import { t as collectChannelStatusIssues } from "./channels-status-issues-NIbK8RIj.js";
import { d as resolveSystemNodeInfo, l as renderSystemNodeWarning, n as gatewayInstallErrorHint, t as buildGatewayInstallPlan } from "./daemon-install-helpers-DhiLFnDu.js";
...
import { a as launchAgentPlistExists, i as isLaunchAgentLoaded, o as repairLaunchAgentBootstrap, r as isLaunchAgentListed, s as resolveGatewayLogPaths, t as resolveGatewayService } from "./service-D__3wqjc.js";
import { i as auditGatewayServiceConfig, n as renderSystemdUnavailableHints, o as needsNodeRuntimeMigration, r as SERVICE_AUDIT_CODES, s as formatRuntimeStatus, t as isSystemdUnavailableDetail } from "./systemd-hints-Dg-DEV3V.js";
import { t as readLastGatewayErrorLine } from "./diagnostics-vsfOfxSq.js";
import { n as renderGatewayServiceCleanupHints, t as findExtraGatewayServices } from "./inspect-Dnj5VM7D.js";
import { r as healthCommand } from "./health-CMi2oIes.js";
import { n as resolveControlUiDistIndexHealth, r as resolveControlUiDistIndexPathForRoot } from "./control-ui-assets-CgsicWFm.js";
...
import { n as detectLegacyStateMigrations, r as runLegacyStateMigrations, t as loadAndMaybeMigrateDoctorConfig } from "./doctor-config-flow-CqJzrB-I.js";
import { t as ensureSystemdUserLingerInteractive } from "./systemd-linger-CWMm3E4l.js";
import { t as formatHealthCheckFailure } from "./health-format-DcHcIWyi.js";
import { n as doctorShellCompletion } from "./doctor-completion-DUhq6oWB.js";
...
async function maybeRepairLaunchAgentBootstrap(params) {
	...
	note$1("LaunchAgent 已列出但未在 launchd 中加载。", `${params.title} LaunchAgent`);
	...
	note$1(`${params.title} LaunchAgent 已修复。`, `${params.title} LaunchAgent`);
	return true;
}
async function maybeRepairGatewayDaemon(params) {
	...
	if (params.cfg.gateway?.mode !== "remote") {
		...
		if (diagnostics.status === "busy") note$1(formatPortDiagnostics(diagnostics).join("\n"), "网关端口");
			...
			if (lastError) note$1(`最近网关错误: ${lastError}`, "网关");
		}
	}
	if (!loaded) {
		...
		if (await params.prompter.confirmSkipInNonInteractive({
			...
			try {
				...
				if (String(err).includes("gateway closed")) {
					note$1("网关未运行。", "网关");
					note$1(params.gatewayDetailsMessage, "网关连接");
...
async function checkGatewayHealth(params) {
	...
	try {
		...
		if (String(err).includes("gateway closed")) {
			note$1("网关未运行。", "网关");
			note$1(gatewayDetails.message, "网关连接");
		} else params.runtime.error(formatHealthCheckFailure(err));
	}
	if (healthOk) try {
...
async function noteSecurityWarnings(cfg) {
	...
	if (cfg.approvals?.exec?.enabled === false) warnings.push("- 注意: approvals.exec.enabled=false 仅禁用审批转发。", "  主机执行控制仍来自 ~/.openclaw/exec-approvals.json。", `  Check local policy with: ${formatCliCommand("openclaw approvals get --gateway")}`);
	...
	if (isExposed) if (!hasSharedSecret) {
		...
		warnings.push(`- 严重警告: 网关绑定到 ${bindDescriptor} 但未设置认证。`, `  你网络上的任何人（如果做了端口转发则是互联网上的任何人）都可以完全控制你的 AI 助手。`, `  Fix: ${formatCliCommand("openclaw config set gateway.bind loopback")}`, ...saferRemoteAccessLines, ...authFixLines);
	} else warnings.push(`- WARNING: Gateway bound to ${bindDescriptor} (可被网络访问)。`, `  请确保你的认证凭据足够强且未被泄露。`, ...saferRemoteAccessLines);
...
async function noteStateIntegrity(cfg, prompter, configPath) {
	...
	if (!stateDirExists) {
		warnings.push(`- 严重：状态目录缺失 (${displayStateDir})。会话、凭据、日志和配置都存储在那里。`);
		if (cfg.gateway?.mode === "remote") warnings.push("- 网关处于远程模式；请在运行网关的远程主机上执行 doctor。");
		if (await prompter.confirmSkipInNonInteractive({
	...
	if (stateDirExists && process.platform !== "win32") try {
		...
		if (!(isDirSymlink ? fs.realpathSync(stateDir) : stateDir).startsWith("/nix/store/") && (stat.mode & 63) !== 0) {
			warnings.push(`- 状态目录权限过于开放 (${displayStateDir})。建议 chmod 700。`);
			if (await prompter.confirmSkipInNonInteractive({
	...
	if (stateDirExists) {
		...
		if (requireOAuthDir) dirCandidates.set(oauthDir, "OAuth 目录");
	...
	if (entries.length > 0) {
		...
		if (mainEntry?.sessionId) {
			...
			if (!existsFile(transcriptPath)) warnings.push(`- 主会话记录缺失 (${shortenHomePath(transcriptPath)})。历史记录将显示为已重置。`);
			else {
				const lineCount = countJsonlLines(transcriptPath);
				if (lineCount <= 1) warnings.push(`- 主会话记录只有 ${lineCount} 行。会话历史可能没有正常追加。`);
			}
		}
	}
...
function noteWorkspaceStatus(cfg) {
	...
	if (legacyWorkspace.legacyDirs.length > 0) note$1(formatLegacyWorkspaceWarning(legacyWorkspace), "额外工作区");
	const skillsReport = buildWorkspaceSkillStatus(workspaceDir, { config: cfg });
	note$1([
		`可用: ${skillsReport.skills.filter((s) => s.eligible).length}`,
		`缺少依赖: ${skillsReport.skills.filter((s) => !s.eligible && !s.disabled && !s.blockedByAllowlist).length}`,
...
async function doctorCommand(runtime = defaultRuntime, options = {}) {
	...
	if (!cfg.gateway?.mode) {
		...
		if (!fs.existsSync(configPath)) lines.push(`缺少配置：请先运行 ${formatCliCommand("openclaw setup")}。`);
		note$1(lines.join("\n"), "网关");
	}
	...
	if (gatewayDetails.remoteFallbackNote) note$1(gatewayDetails.remoteFallbackNote, "网关");
	if (resolveMode(cfg) === "local" && sourceConfigValid) {
		...
		if (auth.mode !== "password" && (auth.mode !== "token" || !auth.token)) {
			note$1("网关认证已关闭或缺少令牌。令牌认证现在是推荐的默认方式（包括本地回环）。", "网关认证");
